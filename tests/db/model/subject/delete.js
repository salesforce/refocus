...
describe('tests/db/model/subject/delete.js >', () => {
  ...
  describe('cascade delete samples >', () => {
    ...
    it('samples deleted when subject is deleted', (done) => {
      ...
      redisOps.getAspSubjMapMembers(aspectToCreate.name)
      .then((res) => {
        expect(res).to.deep.equal(['___subject']);
      }).then(() => redisOps.getSubjAspMapMembers(subject.absolutePath))
      .then((res) => {
      ...
      .then((samp) => {
        ...
        return redisOps.getAspSubjMapMembers(aspectToCreate.name);
      })
      .then((res) => {
