 ...
 */
function getSamplesAndAspects(sampleKeys, redisClient) {
  const commands = [];
...
function sscanAndFilterSampleKeys(cursor, filteredSamples, opts, redisClient) {
  return redisClient.sscanAsync(constants.indexKey.sample, cursor,
    ...
    .then((reply) => {
      ...
      return sscanAndFilterSampleKeys(newCursor, filteredSamples, opts, redisClient);
    });
}
...
module.exports = {
    ...
    .then(redisCache => {
      ...
      if (featureToggles.isFeatureEnabled('optimizeSampleFilteredGets')) {
        ...
        if (shouldGetAllKeys) {
          ...
          return redisClient.sMembers(sampleStore.constants.indexKey.sample)
            .then((sampleKeys) => {
              sampleKeys.sort();
              ...
              const filteredSampleKeys = modelUtils.applyLimitAndOffset(
                opts, sampleKeys);
              return getSamplesAndAspects(filteredSampleKeys, redisClient);
            })
        ...
        if (hasNameFilterOnly && !opts.filter.name.includes('*')) {
          ...
          return Promise.resolve()
            .then(() => {
              ...
              return getSamplesAndAspects(sampleKeys, redisClient);
            })
            .then((samplesAndAspects) => {
        ...
        return Promise.resolve()
          .then(() => {
            ...
            return sscanAndFilterSampleKeys('0', filteredSampleKeys, opts, redisClient);
          })
          .then((filteredKeys) => {
            ...
            return getSamplesAndAspects(keys, redisClient);
          })
          .then((redisResponses) => {
  ...
  upsertSample(newSample, user) {
    ...
    .then((sampleAttributesArray) => {
      const sampleAttributes = sampleAttributesArray[0];
